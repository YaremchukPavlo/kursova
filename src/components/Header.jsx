...
function Header() {
  const [selectedRole, setSelectedRole] = useState(localStorage.getItem("userType") || "");

  const handleRoleChange = (event) => {
    const role = event.target.value;
    setSelectedRole(role);
    localStorage.setItem("userType", role !== "Гість" ? role : null);
  };
      ...
      <div className="d-flex justify-content-between align-items-center">
        <div className="header-dropdown">
          <select value={selectedRole} onChange={handleRoleChange}>
            ...
            <option value="Гість">Гість</option>
            <option value="volunteer">Волонтер</option>
            <option value="ZSU">Військовий</option>
          </select>
        </div>
        <p className="m-0">Military Car Helper</p>
        <div className="header-buttons align-items-center">
          ...
          <Link to={selectedRole ? "/user-profile" : "/login"} className="link1">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="30"
              height="30"

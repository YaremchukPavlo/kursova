...
const NewsPage = () => {
  ...
  return (
    <div className="App">
      ...
      <div
        div
        className="home-page row d-flex justify-content-center"
        style={{ backgroundImage: `url(${backgroundImage})` }}
      >
        <CustomSlider items={sliderItems} />
        <div
          className="news-grid col-9 justify-content-center mt-5 mb-5"
          ...
          <ul>
            {Array.isArray(news) &&
              news.map((item) => (
                <li key={item.id}>
                  <img
                    src={`http://192.168.0.103:8040/uploads/news/${item.image}`}
                    className="rounded image-css"
                    alt="image"
                  />
                  <p>{item.title}</p>
                  <p>{item.content}</p>
